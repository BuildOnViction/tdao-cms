...
import { postBroker } from '../store/actions/broker';
...
const createSchema = Yup.object().shape({
    ...
    file: Yup.mixed().test('fileFormat', 'Chỉ được up ảnh', (value) => {
         return value && SUPPORTED_FORMATS.includes(value.type);
    }),

});
class BrokerCreatePage extends React.Component {
                            ...
                            <CardBody>
                                <Formik
                                    ...
                                    onSubmit={async values => {
                                        let data ={name:values.name,email:values.email,phone: values.phone,introduce:values.introduce, dob: values.dob, company:values.company, password: values.password};
                                        await this.props.postBroker(data);
                                        history.push('/brokers/?success=true');
                                    }}
                                >
...
}, {getListCompanies,postBroker})(BrokerCreatePage);
