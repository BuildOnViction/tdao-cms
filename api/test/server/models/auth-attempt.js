...
const config = Config.hapiMongoModels.mongodb;
...
lab.experiment('AuthAttempt Model', () => {
    ...
    lab.test('it detects login abuse from an ip and many users', async () => {
        ...
        const attemptConfig = Config.authAttempts;
        const authRequest = (i) => AuthAttempt.create({ip: '127.0.0.2', username: `mudskipper${i}`});
    ...
    lab.test('it detects login abuse from an ip and one user', async () => {
        ...
        const attemptConfig = Config.authAttempts;
        const authRequest = () => AuthAttempt.create({ip: '127.0.0.3', username: 'steve'});
    ...
    lab.test('it returns a new instance when create succeeds', async () => {
        ...
        const authAttempt = await AuthAttempt.create({ip: '127.0.0.4', username: 'ren'});
